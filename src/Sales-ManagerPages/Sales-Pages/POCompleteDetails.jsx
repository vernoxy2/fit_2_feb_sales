...
export default function POCompleteDetails() {
  ...
  const itemsAnalysis = items.map((item) => {
    ...
    const receiptEvents = invoices.map((inv) => {
      const found = (inv.items || []).find((ii) => ii.productCode === item.productCode);
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="relative">
              ...
              <div className="space-y-6">
                {timelineEvents.map((event, idx) => {
                  ...
                  const gapDays = prevDate && event.date ? daysBetween(prevDate, event.date) : null;
                  return (
        ...
        {/* ── Invoices Received — SIMPLE (no product list) ── */}
        <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
          <div className="px-6 py-4 border-b border-slate-50">
          ...
          <div className="divide-y divide-slate-50">
            {invoices.map((inv, idx) => {
              const unitsThisInv = (inv.items || []).reduce((s, i) => s + (i.newReceived || 0), 0);
                ...
                <div key={inv.id} className={`px-6 py-4 ${isLast ? "bg-emerald-50/40" : ""}`}>
                  ...
                  {inv.qualityCheck && (
                    <div className={`mt-2 inline-flex items-center gap-1 text-[10px] font-bold px-2 py-0.5 rounded-full ${
                      inv.qualityCheck === "passed" ? "bg-emerald-100 text-emerald-700"
                      : inv.qualityCheck === "failed" ? "bg-red-100 text-red-700"
      ...
      <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
        <div className="px-6 py-4 border-b border-slate-50 flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-0.5">Ordered vs received — per invoice breakdown</p>
          </div>
          ...
          <table className="w-full">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-100">
                <th className="text-left px-6 py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider">Product Code</th>
                <th className="text-left px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider">Description</th>
                <th className="text-center px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider">Ordered</th>
                {invoices.map((inv, idx) => (
                  <th key={inv.id} className="text-center px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider">
                    Inv {idx + 1}
                ...
                <th className="text-center px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider">Total Recv</th>
                <th className="text-center px-4 py-3 text-[10px] font-black text-slate-400 uppercase tracking-wider">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-50">
              {itemsAnalysis.map((item, idx) => {
                const progress = item.ordered > 0 ? Math.round((item.received / item.ordered) * 100) : 0;
                const isExact = item.received === item.ordered;
                const isExcess = item.received > item.ordered;
                return (
                  <tr key={idx} className="hover:bg-slate-50/60 transition-colors">
                    <td className="px-6 py-4">
                      <span className="font-mono text-sm font-bold text-slate-700">{item.productCode || "—"}</span>
                    </td>
                    <td className="px-4 py-4 max-w-[200px]">
                      <p className="text-xs text-slate-500 truncate">{item.description || "—"}</p>
                    </td>
                    <td className="px-4 py-4 text-center">
                      <span className="text-sm font-bold text-slate-700">{item.ordered}</span>
                    ...
                    <td className="px-4 py-4 text-center">
                      {isExcess ? <StatusBadge status="excess" /> : <StatusBadge status="complete" />}
                    </td>
                  </tr>
                );
              })}
            ...
            <tfoot>
              <tr className="bg-slate-50 border-t-2 border-slate-200">
                ...
                {invoices.map((inv) => {
                  const invTotal = (inv.items || []).reduce((s, i) => s + (i.newReceived || 0), 0);
                  return (
      ...
      {invoices.some((inv) => inv.qualityCheck) && (
        <div className="bg-white border border-slate-100 rounded-2xl shadow-sm overflow-hidden">
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-4">
            {invoices.map((inv, idx) => (
              <div key={inv.id} className={`p-4 rounded-xl border ${
                inv.qualityCheck === "passed" ? "bg-emerald-50 border-emerald-200"
                : inv.qualityCheck === "failed" ? "bg-red-50 border-red-200"
                : "bg-orange-50 border-orange-200"
              }`}>
                <div className="flex items-center justify-between mb-2">
                  <p className="text-xs font-bold text-slate-700">Invoice #{inv.invoiceNo || idx + 1}</p>
