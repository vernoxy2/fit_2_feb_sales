...
//           if (doc.soStatus==="complete") return true;
//           return true;
//         });
    ...
    <Card>
      <div className="p-10 text-center">
        ...
        <p className="text-sm text-slate-500 mb-8 max-w-md mx-auto">
          Store team is verifying stock availability and quality. This page will{" "}
          <strong>automatically unlock</strong> once they approve.
        </p>
        <div className="flex items-center justify-center gap-3 flex-wrap mb-8">
        ...
        <div className="flex items-start gap-3 p-4 bg-blue-50 border border-blue-200 rounded-xl max-w-sm mx-auto text-left">
          ...
          <p className="text-xs text-blue-700">
            <strong>Store team</strong> will verify from their QC panel. Invoice
            upload will automatically appear here once approved.
          </p>
        </div>
      </div>
    </Card>
...
export default function UploadSalesInvoice() {
  ...
  return (
    <div className="space-y-6">
      ...
      {step === 1 && (
        <Card>
          <CardHeader
            ...
            subtitle={`${pendingSOs.length} SOs`}
          />
          {loadingSOs ? (
            <div className="p-12 text-center">
            ...
            <>
              <div className="divide-y divide-slate-50">
                {paginatedSOs.map((so) => {
                  ...
                  const remaining = totalOrdered - totalInvoiced;
                  const isQCPending = so.status === "waiting_for_qc";
                  const isComplete = so.status === "complete";
                  return (
                    <div
                      key={so.id}
                      className={`px-6 py-4 transition-colors ${
                        isComplete
                          ? "bg-emerald-50/30 cursor-pointer hover:bg-emerald-50/50"
                          : so.status === "partial"
                            ? "bg-orange-50/40 hover:bg-orange-50 cursor-pointer"
                            : isQCPending
                              ? "bg-violet-50/30 cursor-pointer"
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex-1">
                          ...
                          <div className="flex items-center gap-4 mt-1 text-xs text-slate-400">
                            ...
                            {totalInvoiced > 0 && (
                              <span
                                className={`font-bold ${isComplete ? "text-emerald-600" : "text-orange-600"}`}
                              >
                        ...
                        {/* ✅ CHANGE: complete → FiEye button, others → normal */}
                        {isComplete ? (
                          <button
                            className="ml-4 flex items-center gap-1.5 px-3 py-1.5 bg-emerald-100 text-emerald-700 border border-emerald-200 text-xs font-bold rounded-lg hover:bg-emerald-200 transition-colors whitespace-nowrap"
                            onClick={(e) => {
                              e.stopPropagation();
                              handleSelectSO(so);
                            }}
      ...
      {/* ✅ STEP 2 — Complete SO — Read Only View */}
      {step === 2 && selectedSO && selectedSO.status === "complete" && (
        <div className="space-y-6">
          <div className="flex items-center gap-3 p-4 bg-emerald-50 border border-emerald-200 rounded-xl">
            ...
            <div>
              <p className="text-sm font-black text-emerald-800">
                SO Fully Invoiced & Complete
              </p>
              <p className="text-xs text-emerald-600 mt-0.5">
                All units invoiced — no further action required
              </p>
            </div>
          </div>
          <Card>
            <CardHeader
              ...
              subtitle="Read only — already complete"
            />
            <div className="p-6 space-y-4">
              <div className="p-4 bg-slate-50 rounded-lg">
                <div className="grid grid-cols-2 gap-3 text-xs">
      ...
      {/* Action Buttons */}
      {step === 2 && !isWaitingForQC && selectedSO?.status !== "complete" && (
