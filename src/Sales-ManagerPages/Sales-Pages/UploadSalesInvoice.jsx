...
function StatusPill({ status }) {
  ...
  const label =
    s === "ready" || s === "ready_to_dispatch"
      ...
      : s?.replace("_", " ").toUpperCase();
  return (
    <span className={`px-2.5 py-0.5 text-[10px] font-bold rounded-full border uppercase ${map[s] || "bg-slate-50 text-slate-600 border-slate-200"}`}>
      {label}
    </span>
  );
}
...
function SOHistoryTimeline({ selectedSO, linkedInvoices, loadingHistory }) {
  ...
  [...linkedInvoices]
    ...
    .forEach((inv) => {
      ...
      const pending = (inv.items || []).reduce(
        (s, i) => s + Math.max(0, (i.orderedQty || 0) - (i.totalInvoicedQty || 0)), 0,
      );
      events.push({
        type: "invoice", icon: "🧾",
        label: `Sales Invoice Uploaded${inv.invoiceNo ? ` — ${inv.invoiceNo}` : ""}`,
        sub: `${thisQty} units invoiced · Status updated`,
        datetime: inv.createdAt,
        invoiceDate: inv.invoiceDate,
        remarks: inv.remarks,
        status: inv.soStatus || "partial",
      ...
      if (inv.soStatus === "partial") {
        events.push({
          type: "status", icon: "🔄", label: "Status → PARTIAL",
          sub: `${pending} units still pending`,
          datetime: inv.createdAt, status: "partial", pending,
        });
      } else if (inv.soStatus === "ready") {
        events.push({
          type: "status", icon: "✅", label: "Status → READY TO DISPATCH",
          sub: "All units invoiced",
          datetime: inv.createdAt, status: "ready",
        });
      } else if (inv.soStatus === "excess") {
        events.push({
          type: "status", icon: "⚠️", label: "Status → EXCESS",
          sub: "Invoiced more than ordered",
          datetime: inv.createdAt, status: "excess",
        });
      }
    });
      ...
      {loadingHistory ? (
        ...
        <div className="divide-y divide-slate-50">
            ...
            <div key={idx} className="px-6 py-3 flex items-start justify-between gap-4 hover:bg-slate-50/60">
              <div className="flex items-start gap-3 flex-1 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  {event.type === "invoice" && event.invoiceDate && (
                    <span className="text-[10px] text-slate-400">📅 {formatDate(event.invoiceDate)}</span>
                  )}
                  {event.type === "status" && event.status === "partial" && event.pending > 0 && (
                    <p className="text-[10px] text-orange-500 font-bold mt-1">↳ {event.pending} units still pending</p>
                  )}
                </div>
              </div>
              {event.datetime && (
          ...
          {linkedInvoices.length === 0 && (
            <div className="px-6 py-4 flex items-center gap-3">
              <span className="text-sm">⏳</span>
              <p className="text-xs text-slate-400">No invoices uploaded yet for this SO</p>
            </div>
          )}
        </div>
      )}
...
export default function UploadSalesInvoice() {
  ...
  useEffect(() => {
    const fetchSOs = async () => {
      try {
        ...
        const all  = snap.docs.map((d) => ({ id: d.id, ...d.data() }));
        const sos  = all.filter((d) => {
          if (d.type !== "SALES_ORDER") return false;
          if (d.soStatus === "complete")  return false;
          return true;
        });
        const mapped = sos.map((so) => ({
          id:        so.id,
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        const snap = await getDocs(
          query(collection(db, "excelupload"), where("linkedSoId", "==", selectedSO.id)),
        );
        setLinkedInvoices(
          snap.docs
            .map((d) => ({ id: d.id, ...d.data() }))
  ...
  const handleInvoiceExcel = (e) => {
    ...
    reader.onload = (event) => {
      try {
        ...
        if (docType === "PURCHASE_INVOICE") {
          alert("This is a Purchase Invoice! Please use 'Upload Vendor Invoice' instead.");
          setParsingExcel(false);
          return;
        }
        ...
        if (header.invoiceNo) {
          setInvoiceNo(header.invoiceNo);
          const isDupe = linkedInvoices.some(
            (inv) => inv.invoiceNo?.toLowerCase().trim() === header.invoiceNo?.toLowerCase().trim(),
          );
  ...
  // ── Submit — Only status update, NO stock deduct ──────────────────────────
  const handleSubmit = async () => {
    setUploading(true);
    try {
      const now = new Date().toISOString();
      ...
      const soStatus = calcSoStatus(
        updatedItems.map((i) => ({ orderedQty: i.orderedQty, totalInvoicedQty: i.totalInvoicedQty })),
      );

      // 1. Update SO document (status update only — no stock deduct)
      await updateDoc(doc(db, "excelupload", selectedSO.id), {
        items:            updatedItems,
        soStatus,
        lastInvoiceAt:    now,
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">Generate invoice and update SO status</p>
        </div>
        <div className="flex gap-3">
          <BtnPrimary onClick={() => navigate("/sales/sales-orders/upload")}>
            <FiPlus size={14} /> Upload SO
      ...
      {step === 1 && (
        <Card>
          ...
          {loadingSOs ? (
            ...
            <div className="p-12 text-center">
              <FiFileText size={48} className="mx-auto mb-3 text-slate-300" />
              <p className="text-sm font-bold text-slate-600">No Pending Sales Orders</p>
            </div>
          ) : (
            <div className="divide-y divide-slate-50">
              {pendingSOs.map((so) => {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                  ...
                  <div className="space-y-1 max-h-48 overflow-y-auto">
                    {selectedSO.items.map((item, idx) => (
                      <div key={idx} className="flex items-center justify-between text-xs bg-slate-50 px-3 py-2 rounded-lg">
                        ...
                        {item.totalInvoicedQty > 0 && (
                          <span className="text-orange-600 font-bold">Inv: {item.totalInvoicedQty}</span>
                        )}
                      </div>
                    ))}
                  </div>
            ...
            <Card>
              ...
              <div className="p-6 space-y-4">
                <div
                  className="border-2 border-dashed border-slate-200 rounded-xl p-6 text-center hover:border-indigo-300 cursor-pointer"
                  onClick={() => document.getElementById("salesInvoiceInput").click()}
                >
                  <FiUpload size={24} className="mx-auto mb-2 text-slate-400" />
                  <p className="text-sm text-slate-600 font-medium">
                    {invoiceExcelFile ? invoiceExcelFile.name : "Click to upload Sales Invoice Excel"}
                  </p>
                  <p className="text-xs text-slate-400 mt-1">.xlsx or .xls</p>
                  <input
                    id="salesInvoiceInput"
                ...
                {duplicateWarning && (
                  <div className="p-3 bg-red-50 border border-red-200 rounded-lg flex items-start gap-2">
                    <FiAlertTriangle size={14} className="text-red-500 mt-0.5" />
                ...
                {excelParsed && (
                  <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                    <p className="text-xs font-bold text-blue-700">
                      📦 {invoicedItems.filter((i) => i.matchedFromExcel).length} items matched
                    </p>
                    {invoicedItems.filter((i) => !i.matchedFromExcel).length > 0 && (
                      <p className="text-xs text-orange-600 mt-1">
                        ⚠️ {invoicedItems.filter((i) => !i.matchedFromExcel).length} SO items not in Invoice
                      </p>
                    )}
                  </div>
                )}
      ...
      {step === 3 && selectedSO && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="p-6 space-y-4">
              <div className="p-4 bg-slate-50 rounded-lg">
                <div className="grid grid-cols-2 gap-3 text-xs">
                  ...
                  <div>
                    <p className="text-slate-400 font-bold mb-1">Current Status</p>
                    <StatusPill status={selectedSO.status} />
                  </div>
                  <div>
              ...
              <div className="p-3 bg-slate-50 rounded-lg text-xs space-y-1">
                ...
                <div className="flex justify-between">
                  <span className="text-slate-500">This Invoice Qty:</span>
                  <span className="font-bold">{getTotalNewInvoiced()} units</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-slate-500">Still Pending:</span>
              ...
              <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
                <p className="text-xs font-bold text-blue-700 flex items-center gap-1.5">
                  ℹ️ Stock was already deducted when SO was created
                </p>
              </div>
            </div>
          </Card>
          ...
          <Card>
            <CardHeader title="Verify Quantities" subtitle={`${getTotalNewInvoiced()} units this invoice`} />
            <div className="p-6 space-y-3">
              {invoicedItems.map((item, idx) => {
                const ordered   = item.orderedQty || 0;
                const already   = item.alreadyInvoiced || 0;
                ...
                return (
                  <div key={idx} className={`p-4 border rounded-lg ${
                    itemStatus === "ready"   ? "border-emerald-200 bg-emerald-50/30" :
                    ...
                    <div className="grid grid-cols-4 gap-2 mb-3">
                      ...
                      <div>
                        <p className="text-[10px] text-slate-400 font-bold mb-1">Prior Inv</p>
                      ...
                      <div>
                        ...
                        <p className={`text-sm font-bold ${
                          remaining > 0 ? "text-orange-600" : excess > 0 ? "text-purple-600" : "text-emerald-600"
                        }`}>
                    ...
                    <div className="w-full bg-slate-100 rounded-full h-1.5">
                      <div
                        className={`h-1.5 rounded-full transition-all ${
                          itemStatus === "ready"   ? "bg-emerald-500" :
                          itemStatus === "excess"  ? "bg-purple-500"  :
                          itemStatus === "partial" ? "bg-orange-500"  : "bg-blue-300"
                        }`}
                        style={{ width: `${Math.min(pct, 100)}%` }}
                      />
                    </div>
              ...
              {getTotalShortage() > 0 && (
                <div className="p-3 bg-orange-50 border border-orange-200 rounded-lg">
                  <p className="text-xs font-bold text-orange-700 flex items-center gap-1.5">
                    <FiAlertTriangle size={12} /> {getTotalShortage()} units pending — SO will be: PARTIAL
                  </p>
                </div>
              )}
              {liveSoStatus === "ready" && (
                <div className="p-3 bg-emerald-50 border border-emerald-200 rounded-lg">
                  <p className="text-xs font-bold text-emerald-700 flex items-center gap-1.5">
                    <FiCheck size={12} /> All matched — SO will be: READY TO DISPATCH
                  </p>
                </div>
              )}
              {liveSoStatus === "excess" && (
      ...
      {step === 4 && selectedSO && (
        <Card>
          <CardHeader title="Confirm Invoice" subtitle="SO status will be updated" />
          <div className="p-6 max-w-lg space-y-4">
            <Textarea
              label="Remarks (optional)"
              value={remarks}
            ...
            <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
              <p className="text-xs font-bold text-blue-700">
                ℹ️ Stock was deducted at SO creation — only status will update
              </p>
            </div>
          </div>
        </Card>
      ...
      {step === 2 && (
        <div className="flex justify-end gap-3">
          <BtnSecondary onClick={() => setStep(1)}>← Back</BtnSecondary>
          <BtnPrimary onClick={() => setStep(3)} disabled={!excelParsed || !invoiceNo}>
            Next: Verify Quantities →
          </BtnPrimary>
        </div>
      )}
      ...
      {step === 4 && (
        <div className="flex justify-end gap-3">
          <BtnSecondary onClick={() => setStep(3)}>← Back</BtnSecondary>
          <BtnPrimary onClick={handleSubmit} disabled={uploading}>
            {uploading ? "Processing..." : "Confirm Invoice →"}
          </BtnPrimary>
        </div>
      )}
        ...
        <Card>
          <div className="p-12 text-center">
            ...
            <div className="space-y-1.5 text-sm text-slate-600 mb-8">
              ...
              <p>✅ SO status updated to <strong>{liveSoStatus?.toUpperCase()}</strong></p>
              <p>✅ Invoice #{linkedInvoices.length + 1} for this SO</p>
              {getTotalShortage() > 0 && (
                <p className="text-orange-600 font-bold">⚠️ {getTotalShortage()} units still pending</p>
              )}
            </div>
            <div className="flex items-center justify-center gap-3 flex-wrap">
              {getTotalShortage() > 0 && (
                <BtnPrimary onClick={() => window.location.reload()}>Upload Remaining Invoice</BtnPrimary>
              )}
              <BtnSecondary onClick={() => {
                setStep(1); setSelectedSO(null); setInvoicedItems([]);
                setExcelParsed(false); setInvoiceExcelFile(null);
                setInvoiceNo(""); setInvoiceHeader(null);
                setLinkedInvoices([]); setDuplicateWarning("");
              }}>
                Upload Another Invoice
              </BtnSecondary>
              <BtnPrimary onClick={() => navigate("/sales/sales-orders")}>
