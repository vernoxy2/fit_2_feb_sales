  ...
  return (
    <div className="px-6 py-4 border-t border-slate-100 flex items-center justify-between">
      <p className="text-xs text-slate-400">
        Showing{" "}
        <span className="font-bold text-slate-600">{start}–{end}</span>{" "}
        of <span className="font-bold text-slate-600">{totalItems}</span> {label}
      </p>
      <div className="flex items-center gap-1">
        <button
          onClick={() => onPageChange(Math.max(1, currentPage - 1))}
          disabled={currentPage === 1}
          className="w-8 h-8 flex items-center justify-center rounded-lg border border-slate-200 text-xs font-bold text-slate-500 hover:bg-slate-50 disabled:opacity-30 disabled:cursor-not-allowed"
        >‹</button>
...
function SOHistoryTimeline({ selectedSO, linkedInvoices, loadingHistory }) {
  ...
  const totalOrdered = selectedSO.items.reduce(
    (s, i) => s + (i.orderedQty || i.quantity || 0), 0,
  );
  const totalInvoiced = selectedSO.items.reduce(
    (s, i) => s + (i.totalInvoicedQty || 0), 0,
  );
...
export default function UploadSalesInvoice() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* ════════════════════════════════════════════════════════════════════ */}
      {/* STEP 1 — Select SO (with pagination, 10 per page)                   */}
      {/* ════════════════════════════════════════════════════════════════════ */}
      {step === 1 && (
        <Card>
          <CardHeader
                  ...
                  return (
                    <div
                      ...
                      className={`px-6 py-4 hover:bg-slate-50 cursor-pointer ${
                        so.status === "partial" ? "bg-orange-50/40" : ""
                      }`}
      ...
      {/* ════════════════════════════════════════════════════════════════════ */}
      {/* STEP 3 — Verify Quantities (items paginated 10 per page)             */}
      {/* ════════════════════════════════════════════════════════════════════ */}
      {step === 3 && selectedSO && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="p-3 bg-slate-50 rounded-lg text-xs space-y-1">
                ...
                <div className="flex justify-between">
                  ...
                  <span
                    className={`font-bold ${
                      getTotalShortage() > 0 ? "text-orange-600" : "text-emerald-600"
                    }`}
                  >
              ...
              <div className="p-3 bg-amber-50 border border-amber-200 rounded-lg">
                <p className="text-xs font-bold text-amber-700 flex items-center gap-1.5">
                  <FiAlertTriangle size={12} /> Stock will be deducted on confirm
                </p>
              </div>
            </div>
          </Card>

          {/* Right — Verify Quantities (paginated) */}
          <Card>
            <CardHeader
            ...
            <div className="p-6 space-y-3">
              ...
              {/* Status banners — always visible regardless of page */}
              {getTotalShortage() > 0 && (
      ...
      {step === 2 && (
        <div className="flex justify-end gap-3">
          ...
          <BtnPrimary
            onClick={() => { setItemsPage(1); setStep(3); }}
