...
// ── Stock lookup by Part No ───────────────────────────────────────────────────
async function lookupStock(productCode) {
  try {
    const q = query(collection(db, "stock"), where("productCode", "==", productCode.trim().toUpperCase()));
...
function exportCSV(header, rows, challanNo) {
  const lines = [
    ...
    ["Customer", header.customer], ["SO Reference", header.soReference],
...
export default function DispatchOnChallan() {
  ...
  const [rows, setRows] = useState([
    { productCode: "", description: "", hsn: "", unit: "", dispatchQty: 0, stock: 0, remarks: "", lookingUp: false, found: false },
  ]);
  ...
  const updateRow = async (i, field, value) => {
    ...
    if (field === "productCode") {
      ...
      if (trimmed.length >= 2) {
        setRows((p) => { const n = [...p]; n[i] = { ...n[i], lookingUp: true }; return n; });
        const info = await lookupStock(trimmed);
        setRows((p) => {
          const n = [...p];
          n[i] = {
            ...n[i], lookingUp: false, found: info.found,
        ...
        setRows((p) => { const n = [...p]; n[i] = { ...n[i], found: false }; return n; });
      }
    }
  };

  const getHeader = () => ({
    challanDate, soReference, customer, companyName, address,
    gstin, stateName, email, voucherNo, paymentTerms,
    consignee, destination, invoiceNos, approxInvoiceDate,
    vehicleNo, driverName, transporterName, deliveryNote,
  });

  const canSave = approxInvoiceDate && rows.some((r) => r.productCode && r.dispatchQty > 0);
        ...
        <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <div className="p-6 space-y-5">
            <div className="grid grid-cols-3 gap-3">
              {[["Challan No", challanNo], ["Customer", customer || "—"], ["SO Reference", soReference || "—"],
            ...
            <div className="border border-slate-200 rounded-xl p-5 bg-slate-50">
              ...
              <div className="flex gap-3">
                <button onClick={() => exportPDF(getHeader(), filledRows, challanNo)}
                ...
                <button onClick={() => exportCSV(getHeader(), filledRows, challanNo)}
            ...
            <div className="flex gap-3">
              <button onClick={() => navigate("/sales/dispatch-on-challan/create")}
    ...
    <div className="space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          <BackBtn to="/sales/dispatch-on-challan" label="Back to Challan List" />
          <div>
        ...
        <div className="flex items-center gap-2">
          <span className="text-xs font-mono font-bold text-slate-400 bg-slate-100 px-3 py-1.5 rounded-lg">{challanNo}</span>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
        ...
        <div className="p-5 grid grid-cols-3 gap-4">
          <div>
            <label className="block text-xs font-bold text-slate-500 mb-1 uppercase tracking-wide">Challan No</label>
            <input value={challanNo} readOnly className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-slate-50 font-mono text-slate-500" />
          </div>
          <div>
            <label className="block text-xs font-bold text-slate-500 mb-1 uppercase tracking-wide">Challan Date</label>
            <input value={challanDate} readOnly className="w-full px-3 py-2 text-sm border border-slate-200 rounded-lg bg-slate-50 text-slate-500" />
          </div>
          <Field label="SO / PO Reference" value={soReference} onChange={(e) => setSoReference(e.target.value)} placeholder="e.g. EVFN/2526/02790" />

          <Field label="Customer / Buyer" value={customer} onChange={(e) => setCustomer(e.target.value)} placeholder="Customer / Buyer name" required />
          <div className="col-span-2">
            <Field label="Company Name" value={companyName} onChange={(e) => setCompanyName(e.target.value)} placeholder="Company name" />
          </div>

          <div className="col-span-2">
            <Field label="Address" value={address} onChange={(e) => setAddress(e.target.value)} placeholder="Full address" />
          </div>
          <Field label="GSTIN" value={gstin} onChange={(e) => setGstin(e.target.value)} placeholder="e.g. 24XXXXX" mono />

          <Field label="State" value={stateName} onChange={(e) => setStateName(e.target.value)} placeholder="e.g. Gujarat" />
          <Field label="Email" value={email} onChange={(e) => setEmail(e.target.value)} placeholder="email@example.com" />
          <Field label="Payment Terms" value={paymentTerms} onChange={(e) => setPaymentTerms(e.target.value)} placeholder="e.g. 45 DAYS" />

          <Field label="PO / Voucher No" value={voucherNo} onChange={(e) => setVoucherNo(e.target.value)} placeholder="e.g. 272" mono />
          <div className="col-span-2">
            <Field label="Consignee (Ship To)" value={consignee} onChange={(e) => setConsignee(e.target.value)} placeholder="Consignee name & address" />
          </div>

          <Field label="Destination" value={destination} onChange={(e) => setDestination(e.target.value)} placeholder="e.g. VALSAD" />
          <div>
            <label className="block text-xs font-bold text-slate-500 mb-1 uppercase tracking-wide">Invoice Nos (Unbilled)</label>
            <input value={invoiceNos} onChange={(e) => setInvoiceNos(e.target.value)} placeholder="e.g. F/0716/25-26"
          ...
          <Field label="Approx. Invoice Date" value={approxInvoiceDate} onChange={(e) => setApproxInvoiceDate(e.target.value)} type="date" required />
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
          ...
          <table className="w-full min-w-[820px]">
            <thead>
              <tr className="bg-slate-50 border-b border-slate-200">
                ...
                <th className="px-3 py-2.5 text-left text-xs font-bold text-slate-500 uppercase">Part No <span className="text-indigo-400 normal-case font-normal">(type to auto-fill)</span></th>
                <th className="px-3 py-2.5 text-left text-xs font-bold text-slate-500 uppercase">Description</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {rows.map((row, i) => (
                <tr key={i} className={`${row.found ? "bg-green-50/30" : ""} hover:bg-slate-50/50 transition-colors`}>
                  <td className="px-3 py-2.5 text-sm text-slate-400 text-center">{i + 1}</td>
                  <td className="px-3 py-2.5">
                    <div className="relative">
                      <input value={row.productCode} onChange={(e) => updateRow(i, "productCode", e.target.value)} placeholder="Part No"
                      ...
                      {row.lookingUp && <div className="absolute right-1.5 top-1.5 w-3 h-3 border-2 border-indigo-400 border-t-transparent rounded-full animate-spin" />}
                      {row.found && !row.lookingUp && <span className="absolute right-1.5 top-1.5 text-green-500 text-xs">✓</span>}
                    </div>
                  </td>
                  <td className="px-3 py-2.5">
                    <input value={row.description} onChange={(e) => updateRow(i, "description", e.target.value)} placeholder="Auto-filled from Part No"
                  ...
                  <td className="px-3 py-2.5">
                    <input value={row.hsn} onChange={(e) => updateRow(i, "hsn", e.target.value)} placeholder="HSN"
                      className={`w-24 px-2 py-1.5 text-xs border rounded font-mono focus:outline-none focus:ring-2 focus:ring-indigo-200
                        ${row.found ? "border-green-200 bg-green-50/50" : "border-slate-200 bg-slate-50"}`} />
                  </td>
                  <td className="px-3 py-2.5">
                    <input value={row.unit} onChange={(e) => updateRow(i, "unit", e.target.value)} placeholder="pcs"
                  ...
                  <td className="px-3 py-2.5 text-center">
                    <span className={`text-sm font-bold ${(row.stock ?? 0) > 0 ? "text-green-600" : "text-red-400"}`}>
                      {row.found ? (row.stock ?? 0) : "—"}
                    </span>
                  </td>
                  <td className="px-3 py-2.5">
                    <input type="number" min={0} value={row.dispatchQty || ""} onChange={(e) => updateRow(i, "dispatchQty", Number(e.target.value))} placeholder="0"
                  ...
                  <td className="px-3 py-2.5 text-center">
                    {rows.length > 1 && (
                      <button onClick={() => removeRow(i)} className="text-red-300 hover:text-red-500 text-xl leading-none transition-colors">×</button>
                    )}
                  </td>
                </tr>
              ))}
        ...
        <div className="px-5 py-3 border-t border-slate-100 flex items-center justify-between bg-slate-50">
          <button onClick={addRow} className="flex items-center gap-1.5 text-sm text-indigo-600 hover:text-indigo-800 font-semibold transition-colors">
      ...
      <details className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden group">
        ...
        <div className="p-5 grid grid-cols-3 gap-4">
          <Field label="Vehicle No"  value={vehicleNo}       onChange={(e) => setVehicleNo(e.target.value)}       placeholder="GJ-06-AB-1234" />
          <Field label="Driver Name" value={driverName}      onChange={(e) => setDriverName(e.target.value)}      placeholder="Driver name" />
          <Field label="Transporter" value={transporterName} onChange={(e) => setTransporterName(e.target.value)} placeholder="Transporter name" />
      ...
      <div className="flex items-center justify-between pt-1 pb-6">
        ...
        <div className="flex items-center gap-3">
          {!approxInvoiceDate && <p className="text-xs text-amber-600 font-semibold">⚠ Set Approx. Invoice Date</p>}
          {!rows.some((r) => r.productCode && r.dispatchQty > 0) && (
            <p className="text-xs text-amber-600 font-semibold">⚠ Add at least 1 item</p>
          )}
