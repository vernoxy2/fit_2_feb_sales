...
function toInputDate(val) {
  ...
  if (monShort) {
    const months={jan:"01",feb:"02",mar:"03",apr:"04",may:"05",jun:"06",jul:"07",aug:"08",sep:"09",oct:"10",nov:"11",dec:"12"};
    const m=months[monShort[2].toLowerCase()]; const yr=monShort[3].length===2?"20"+monShort[3]:monShort[3];
    if (m) return `${yr}-${m}-${monShort[1].padStart(2,"0")}`;
  }
  if (/^\d{5}$/.test(s)) { const d=new Date(Math.round((+s-25569)*86400*1000)); if(!isNaN(d)) return d.toISOString().split("T")[0]; }
...
function StatusPill({ status }) {
  const map={
    ...
    received:"bg-teal-50 text-teal-700 border-teal-200",waiting_qc:"bg-indigo-50 text-indigo-700 border-indigo-200",
  };
  const n=status?.toLowerCase();
  const label=n==="waiting_qc"?"Waiting QC":n?.replace("_"," ");
  return <span className={`inline-flex items-center px-2.5 py-0.5 text-[10px] font-bold rounded-full border uppercase tracking-wide ${map[n]||map.pending}`}>{label}</span>;
}
...
function ReceiptProgress({ received, total }) {
  ...
  return (
    <div className="flex items-center gap-2 min-w-0">
      <div className="flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden"><div className={`h-full rounded-full ${color}`} style={{width:`${pct}%`}}/></div>
      <span className={`text-[11px] font-bold tabular-nums whitespace-nowrap ${text}`}>{received}/{total}</span>
    </div>
  );
}
...
function CalendarPicker({ activeDates, selected, onChange }) {
  ...
  const handleDay=(day)=>{
    const iso=`${view.year}-${String(view.month+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;
    if(!activeSet.has(iso))return; onChange(selected===iso?"all":iso); setOpen(false);
  };
  const cells=[];
  for(let i=0;i<firstDayOfWk;i++)cells.push(null);
  for(let d=1;d<=daysInMonth;d++)cells.push(d);
  ...
  return (
    <div className="relative" ref={ref}>
      <button onClick={()=>setOpen(o=>!o)} className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-semibold transition-all shadow-sm border ${selected!=="all"?"bg-indigo-600 text-white border-indigo-600 hover:bg-indigo-700":"bg-white text-slate-700 border-slate-200 hover:border-indigo-300 hover:bg-slate-50"}`}>
        <FiCalendar size={14}/>{btnLabel}
        {selected!=="all"&&<span onClick={(e)=>{e.stopPropagation();onChange("all");}} className="ml-1 w-4 h-4 rounded-full bg-white/25 hover:bg-white/50 flex items-center justify-center text-white text-xs" title="Clear">×</span>}
      </button>
      {open&&(
        <div className="absolute right-0 top-full mt-2 w-72 bg-white border border-slate-200 rounded-2xl shadow-2xl z-50 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 bg-indigo-600">
          ...
          <div className="grid grid-cols-7 px-3 pt-3 pb-1">{DAYS_CAL.map(d=><div key={d} className="text-center text-[10px] font-bold text-slate-400">{d}</div>)}</div>
          <div className="grid grid-cols-7 gap-y-1 px-3 pb-3">
            {cells.map((day,idx)=>{
              if(!day)return<div key={idx}/>;
              const iso=`${view.year}-${String(view.month+1).padStart(2,"0")}-${String(day).padStart(2,"0")}`;
              const hasData=activeSet.has(iso); const isSel=selected===iso; const isToday=iso===today;
              return(
                <button key={idx} onClick={()=>handleDay(day)} disabled={!hasData}
                  ...
                  {day}{hasData&&!isSel&&<span className="absolute bottom-0.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-indigo-400"/>}
                </button>
              );
            })}
          </div>
          <div className="border-t border-slate-100 px-4 py-2.5 flex items-center justify-between">
            ...
            {selected!=="all"&&<button onClick={()=>{onChange("all");setOpen(false);}} className="text-xs text-indigo-600 font-semibold hover:underline">Show all</button>}
          </div>
        </div>
      )}
    </div>
...
function POSelectCard({ po, onSelect }) {
  ...
  const totalReceived=po.items.reduce((s,i)=>s+(i.totalReceivedQty||0),0);
  const remaining=totalOrdered-totalReceived;
  const isDamage=po.hasDamagedPending; const isWaiting=po.status==="waiting_qc"; const isOverdue=po.status==="overdue";
  const accentBar=isDamage?"bg-red-500":isWaiting?"bg-indigo-500":isOverdue?"bg-red-400":po.status==="partial"?"bg-orange-400":"bg-slate-300";
  const btnStyle=isDamage?"bg-red-600 hover:bg-red-700":"bg-indigo-600 hover:bg-indigo-700";
  const btnLabel=isDamage?"Upload Replacement Invoice":po.status==="partial"?"Receive Remaining":"Receive Material";
  return (
      ...
      <div className="pl-4 pr-4 pt-3 pb-3">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2 flex-wrap min-w-0">
            <span className="text-sm font-black text-slate-800">{po.poNumber}</span>
            <StatusPill status={po.status}/>
            {isDamage&&<span className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-red-100 border border-red-200 rounded-full text-[10px] font-bold text-red-700"><FiAlertTriangle size={8}/> {po.totalDamagedQty} Dmg</span>}
            {po.storeQcStatus==="approved_with_issues"&&!isDamage&&<span className="inline-flex items-center gap-1 px-1.5 py-0.5 bg-amber-100 border border-amber-200 rounded-full text-[10px] font-bold text-amber-700"><FiAlertTriangle size={8}/> QC Issues</span>}
            <span className="text-[10px] text-slate-600 flex items-center gap-1"><FiCalendar size={8}/>{formatDateTime(po.createdAt)}</span>
          </div>
          <button className={`flex-shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-white text-[11px] font-bold ${btnStyle} transition-colors`} onClick={(e)=>{e.stopPropagation();onSelect(po);}}>
            {btnLabel} <FiChevronRight size={11}/>
          </button>
        </div>
        <div className="mt-2.5 flex items-center gap-4 text-[11px] text-slate-500 flex-wrap">
          <span className="flex items-center gap-1"><FiTruck size={10} className="text-slate-400"/><span className="font-semibold text-slate-700">{po.vendor}</span></span>
          <span className="flex items-center gap-1"><FiBox size={10} className="text-slate-400"/>{po.items.length} items</span>
          <span className="flex items-center gap-1"><FiClock size={10} className="text-slate-400"/><span className={isOverdue?"text-red-500 font-bold":po.status==="warning"?"text-orange-500 font-bold":""}>ETA: {po.eta!=="—"?po.eta:"Not set"}</span></span>
          <div className="flex items-center gap-1.5 flex-1 min-w-32"><ReceiptProgress received={totalReceived} total={totalOrdered}/></div>
        </div>
        {(isDamage||(!isDamage&&po.status==="partial"&&remaining>0)||isWaiting)&&(
          <div className="mt-2 flex flex-col gap-1">
            {isDamage&&<div className="flex items-center gap-2 px-2.5 py-1.5 bg-red-50 border border-red-200 rounded-lg"><FiAlertTriangle size={11} className="text-red-500 flex-shrink-0"/><p className="text-[11px] font-bold text-red-700">{po.totalDamagedQty} damaged units require replacement invoice</p></div>}
            {!isDamage&&po.status==="partial"&&remaining>0&&<div className="flex items-center gap-2 px-2.5 py-1.5 bg-orange-50 border border-orange-200 rounded-lg"><FiPackage size={11} className="text-orange-500 flex-shrink-0"/><p className="text-[11px] font-bold text-orange-700">{remaining} units still pending from vendor</p></div>}
            {isWaiting&&<div className="flex items-center gap-2 px-2.5 py-1.5 bg-indigo-50 border border-indigo-200 rounded-lg"><span className="text-xs">⏳</span><p className="text-[11px] font-bold text-indigo-700">Invoice submitted — waiting for store QC approval</p></div>}
          </div>
        )}
      </div>
    </div>
    ...
    <Card>
      ...
      {loadingHistory?(
        ...
        <div className="divide-y divide-slate-50">
          {events.map((event,idx)=>(
            <div key={idx} className="px-6 py-3 flex items-start justify-between gap-4 hover:bg-slate-50/60 transition-colors">
              <div className="flex items-start gap-3 flex-1 min-w-0">
                <span className="text-sm mt-0.5 flex-shrink-0">{event.icon}</span>
                <div className="min-w-0">
                  <div className="flex items-center gap-2 flex-wrap"><p className="text-xs font-bold text-slate-800">{event.label}</p><StatusPill status={event.status}/></div>
                  <p className="text-[11px] text-slate-500 mt-0.5">{event.sub}</p>
                  {event.type==="invoice"&&(<div className="flex items-center gap-3 mt-1 flex-wrap">
                    {event.invoiceDate&&<span className="text-[10px] text-slate-400">📅 {formatDate(event.invoiceDate)}</span>}
                  ...
                  {event.type==="invoice"&&event.damagedItems?.length>0&&<div className="mt-1 space-y-0.5">{event.damagedItems.map((d,i)=><span key={i} className="text-[10px] text-red-500 font-bold block">🔴 {d.productCode}: {d.damagedQty} damaged{d.issueDetail?` — ${d.issueDetail}`:""}</span>)}</div>}
                  {event.type==="status"&&event.status==="partial"&&event.pending>0&&<p className="text-[10px] text-orange-500 font-bold mt-1">↳ Next invoice required for remaining {event.pending} units</p>}
                </div>
              </div>
          ...
          {totalPending>0&&linkedInvoices.length>0&&<div className="px-6 py-3 flex items-center gap-3"><span className="text-sm flex-shrink-0">⏳</span><div><p className="text-xs font-bold text-slate-400">Awaiting next invoice...</p><p className="text-[11px] text-orange-500 font-bold mt-0.5">{totalPending} units still pending</p></div></div>}
          {linkedInvoices.length===0&&<div className="px-6 py-4 flex items-center gap-3"><span className="text-sm">⏳</span><p className="text-xs text-slate-400">No invoices uploaded yet for this PO</p></div>}
        </div>
      )}
    </Card>
  );
  ...
  return (
    <div className="space-y-5">
      <Card>
        <div className="p-10 flex flex-col items-center text-center">
          <div className="relative mb-6">
            <div className="w-20 h-20 rounded-full bg-indigo-50 border-2 border-indigo-100 flex items-center justify-center"><FiShield size={34} className="text-indigo-400"/></div>
            <span className="absolute inset-0 rounded-full border-2 border-indigo-300 animate-ping opacity-30"/>
            <span className="absolute -inset-2 rounded-full border border-indigo-200 animate-ping opacity-20" style={{animationDelay:"0.4s"}}/>
          </div>
          <h3 className="text-base font-black text-slate-800 mb-2">Waiting for Store QC Approval{dots}</h3>
          <p className="text-xs text-slate-500 max-w-sm leading-relaxed mb-7">The store team is currently verifying the received material quality. This page will <strong>automatically unlock</strong> once they approve — no need to refresh.</p>
          <div className="flex items-center gap-3 flex-wrap justify-center mb-8">
            {[["PO",selectedPO?.poNumber],["Invoice",invoiceNo||"—"],["Units",getTotalNewReceived()]].map(([label,val])=>(
              <div key={label} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-full">
                <span className="text-[10px] text-slate-400 font-bold uppercase">{label}</span>
...
export default function UploadVendorInvoice() {
      ...
      try {
        ...
        setPendingPOs(pos.map(po=>{
          ...
          return{
            ...
            vendor:po.customer||po.excelHeader?.supplier||po.excelHeader?.consignee||"—",
            date:po.excelHeader?.dated||po.excelHeader?.date||po.excelHeader?.invoiceDate||(po.createdAt?formatDateLabel(po.createdAt):""),
            eta:po.deliveryDate||"—",status:po.storeQcPending?"waiting_qc":po.poStatus||etaStatus,
            remainingDays,createdAt:po.createdAt||null,storeQcStatus:po.storeQcStatus||null,
            totalDamagedQty,hasDamagedPending:totalDamagedQty>0,
            items:(po.items||[]).map(item=>({...item,orderedQty:item.orderedQty||item.quantity||0,totalReceivedQty:item.totalReceivedQty||item.receivedQty||0,unit:item.unit||"pcs"})),
          };
        }));
      } catch(err){void err;}
  ...
  useEffect(()=>{if(loadingPOs||pendingPOs.length===0||!urlPoId)return;const matched=pendingPOs.find(po=>po.id===urlPoId);if(matched)handleSelectPO(matched);},[loadingPOs,pendingPOs.length]);

  useEffect(()=>{
    if(!selectedPO){setLinkedInvoices([]);return;}
    setLoadingHistory(true);
    getDocs(query(collection(db,"excelupload"),where("linkedPoId","==",selectedPO.id))).then(snap=>{setLinkedInvoices(snap.docs.map(d=>({id:d.id,...d.data()})).filter(d=>d.type==="INVOICE").sort((a,b)=>new Date(a.createdAt)-new Date(b.createdAt)));}).catch(()=>{}).finally(()=>setLoadingHistory(false));
  },[selectedPO?.id]);

  useEffect(()=>{
    if(!selectedPO||loadingHistory||savedInvoiceId)return;
    const pendingInv=linkedInvoices.find(inv=>inv.storeQcStatus==="pending");
    if(pendingInv){setInvoiceNo(pendingInv.invoiceNo||"");setInvoiceDate(pendingInv.invoiceDate||new Date().toISOString().split("T")[0]);setSavedInvoiceId(pendingInv.id);setStep(4);}
  },[linkedInvoices,loadingHistory,selectedPO]);
  ...
  useEffect(()=>{
    ...
    if(unsubQcRef.current)unsubQcRef.current();
    setStoreQcStatus("pending");
    const unsub=onSnapshot(doc(db,"excelupload",savedInvoiceId),(snap)=>{
      if(!snap.exists())return;
      const data=snap.data();
      setStoreQcStatus(data.storeQcStatus||"pending");
      setStoreQcApprovedBy(data.storeQcApprovedBy||"");
  ...
  const handleSelectPO=(po)=>{
    ...
    setExcelParsed(false);setInvoiceExcelFile(null);setInvoiceHeader(null);setInvoiceNo("");setDuplicateWarning("");setSavedInvoiceId(null);setStoreQcStatus(null);setStep(2);
  };
  ...
  const handleInvoiceExcel=(e)=>{
    ...
    reader.onload=(event)=>{
      try{
        ...
        const range=XLSX.utils.decode_range(sheet["!ref"]);
        const findVal=(keywords)=>{
          for(let row=0;row<=Math.min(40,range.e.r);row++){for(let col=0;col<=range.e.c;col++){const cell=sheet[XLSX.utils.encode_cell({r:row,c:col})];if(cell&&cell.v){const val=String(cell.v).toLowerCase();for(const kw of keywords){if(val.includes(kw.toLowerCase())){const right=sheet[XLSX.utils.encode_cell({r:row,c:col+1})];const below=sheet[XLSX.utils.encode_cell({r:row+1,c:col})];const right2=sheet[XLSX.utils.encode_cell({r:row,c:col+2})];if(right&&right.v)return String(right.v);if(below&&below.v)return String(below.v);if(right2&&right2.v)return String(right2.v);}}}}}return"";
        };
        const rawSupplierInv=findVal(["Supplier Invoice No.","Supplier Invoice No. & Date","Supplier Inv"]);
        let parsedInvoiceNo="",parsedInvoiceDate="";
        if(rawSupplierInv){const dtMatch=rawSupplierInv.match(/^(.+?)\s+dt\.?\s*(.+)$/i);if(dtMatch){parsedInvoiceNo=dtMatch[1].trim();parsedInvoiceDate=dtMatch[2].trim();}else{parsedInvoiceNo=rawSupplierInv.trim();}}
        ...
        const header={invoiceNo:parsedInvoiceNo,dated:parsedInvoiceDate,supplier:findVal(["Supplier (Bill from)","Supplier","Bill from"]),consignee:findVal(["Consignee (Ship to)","Consignee","Ship to"]),gstin:findVal(["GSTIN/UIN","GSTIN"])};
        if(header.invoiceNo){setInvoiceNo(header.invoiceNo);if(linkedInvoices.some(inv=>inv.invoiceNo?.toLowerCase().trim()===header.invoiceNo?.toLowerCase().trim()))setDuplicateWarning(`⚠️ Invoice "${header.invoiceNo}" has already been uploaded for this PO.`);}
        if(header.dated){const c=toInputDate(header.dated);setInvoiceDate(c||header.dated);}
        setInvoiceHeader(header);
        let tableStartRow=-1;
        for(let row=0;row<=range.e.r;row++){for(let col=0;col<=range.e.c;col++){const cell=sheet[XLSX.utils.encode_cell({r:row,c:col})];if(cell&&cell.v){const val=String(cell.v).toLowerCase();if(val.includes("description of goods")||val==="sl"||val==="si"){tableStartRow=row;break;}}}if(tableStartRow!==-1)break;}
        if(tableStartRow===-1){alert("Table not found");setParsingExcel(false);return;}
        let descCol=-1,hsnCol=-1,partCol=-1,qtyCol=-1;
        for(let col=0;col<=range.e.c;col++){const cell=sheet[XLSX.utils.encode_cell({r:tableStartRow,c:col})];if(cell&&cell.v){const val=String(cell.v).toLowerCase();if(val.includes("description"))descCol=col;if(val.includes("hsn"))hsnCol=col;if(val.includes("part"))partCol=col;if(val.includes("quantity"))qtyCol=col;}}
        const invoiceItems=[];
        for(let row=tableStartRow+2;row<=range.e.r;row++){const descCell=sheet[XLSX.utils.encode_cell({r:row,c:descCol})];if(!descCell||!descCell.v)break;const partCode=partCol>=0?sheet[XLSX.utils.encode_cell({r:row,c:partCol})]?.v||"":"";const qty=qtyCol>=0?parseFloat(sheet[XLSX.utils.encode_cell({r:row,c:qtyCol})]?.v||0):0;invoiceItems.push({productCode:String(partCode).trim(),description:String(descCell.v),invoiceQty:qty,hsnSac:hsnCol>=0?sheet[XLSX.utils.encode_cell({r:row,c:hsnCol})]?.v||"":""});}
        if(selectedPO){setReceivedItems(selectedPO.items.map(poItem=>{const already=poItem.totalReceivedQty||0;const orderedQty=poItem.orderedQty||poItem.quantity||0;const matched=invoiceItems.find(inv=>inv.productCode&&poItem.productCode&&inv.productCode.toLowerCase().trim()===poItem.productCode.toLowerCase().trim());return{...poItem,orderedQty,alreadyReceived:already,newReceived:matched?matched.invoiceQty:0,invoiceQty:matched?matched.invoiceQty:0,matchedFromExcel:!!matched};}));setExcelParsed(true);}
        setParsingExcel(false);
      }catch(err){setParsingExcel(false);alert("Error: "+err.message);}
    };
    reader.readAsArrayBuffer(file);
  };

  const handleInvoiceNoChange=(val)=>{
    setInvoiceNo(val); if(!val.trim()){setDuplicateWarning("");return;}
  ...
  const handleSaveAndWait=async()=>{
    ...
    try{
      const now=new Date().toISOString();
      const updatedItems=receivedItems.map(item=>{const oq=item.orderedQty||0,ar=item.alreadyReceived||0,nr=item.newReceived||0,tr=ar+nr;return{...item,totalReceivedQty:tr,orderedQty:oq,quantity:oq,shortage:Math.max(0,oq-tr),itemStatus:getItemStatus(oq,tr)};});
      const poStatus=calcPoStatus(updatedItems.map(i=>({orderedQty:i.orderedQty,totalReceivedQty:i.totalReceivedQty})));
      const invoiceRef=await addDoc(collection(db,"excelupload"),{type:"INVOICE",linkedPoId:selectedPO.id,linkedPoNo:selectedPO.poNumber,invoiceNo,invoiceDate,vendor:selectedPO.vendor,invoiceHeader:invoiceHeader||{},items:updatedItems,poStatus,invoiceIndex:linkedInvoices.length+1,createdAt:now,storeQcStatus:"pending"});
      await updateDoc(doc(db,"excelupload",selectedPO.id),{storeQcPending:true,pendingInvoiceId:invoiceRef.id});
      setSavedInvoiceId(invoiceRef.id); setUploading(false); setStep(4);
    }catch(err){setUploading(false);alert("Error: "+err.message);}
  };
  ...
  const handleFinalSubmit=async()=>{
    ...
    try{
      ...
      const rawPoStatus=calcPoStatus(updatedItems.map(i=>({orderedQty:i.orderedQty,totalReceivedQty:i.totalReceivedQty})));
      const displayPoStatus=rawPoStatus==="partial"||rawPoStatus==="complete"?"received":rawPoStatus;
      await updateDoc(doc(db,"excelupload",selectedPO.id),{items:updatedItems,poStatus:displayPoStatus,salesConfirmedAt:now,invoiceNo,invoiceNos:arrayUnion(invoiceNo),invoiceDate,qualityCheck,remarks,invoiceCount:linkedInvoices.length+1,totalReceivedQty:updatedItems.reduce((s,i)=>s+i.totalReceivedQty,0),storeQcPending:false,pendingInvoiceId:null});
      if(savedInvoiceId)await updateDoc(doc(db,"excelupload",savedInvoiceId),{qualityCheck,remarks,salesConfirmedAt:now,poStatus:displayPoStatus});
      setUploading(false); setStep(5);
    }catch(err){setUploading(false);alert("Error: "+err.message);}
  };
  ...
  const livePoStatus=(()=>{const c=calcPoStatus(receivedItems.map(i=>({orderedQty:i.orderedQty||0,totalReceivedQty:(i.alreadyReceived||0)+(i.newReceived||0)})));if(c==="partial"||c==="complete")return"received";if(c==="excess")return"excess";return c;})();
  const isStoreApproved=storeQcStatus==="approved"||storeQcStatus==="approved_with_issues";
  const hasIssues=storeQcStatus==="approved_with_issues";
  const damagedItemsList=receivedItems.filter(i=>i.issue==="damage"&&(i.damagedQty||0)>0);
  const steps=[{num:1,label:"Select PO"},{num:2,label:"Upload Invoice"},{num:3,label:"Verify Qty"},{num:4,label:"Submit Invoice"}];
  ...
  return (
    <div className="space-y-6">
      ...
      {step===1&&(
        <div className="space-y-4">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-sm font-black text-slate-800">Select Purchase Order</p>
              <p className="text-xs text-slate-400 mt-0.5">{filteredPOs.length} PO{filteredPOs.length!==1?"s":""}{selectedDate!=="all"?` on ${formatDateLabel(selectedDate)}`:" awaiting material"}</p>
            </div>
            <CalendarPicker activeDates={availableDates} selected={selectedDate} onChange={setSelectedDate}/>
          </div>
          {loadingPOs?(
            <div className="bg-white rounded-2xl border border-slate-200 p-14 text-center"><div className="animate-spin rounded-full h-8 w-8 border-b-2 border-indigo-600 mx-auto mb-3"/><p className="text-sm text-slate-400">Loading purchase orders...</p></div>
          ):filteredPOs.length===0?(
            <div className="bg-white rounded-2xl border border-slate-200 p-14 text-center"><FiCalendar size={40} className="mx-auto mb-3 text-slate-300"/><p className="text-sm font-bold text-slate-600">{selectedDate!=="all"?`${formatDateLabel(selectedDate)} na koi PO nathi`:"No Pending Purchase Orders"}</p><p className="text-xs text-slate-400 mt-1">{selectedDate!=="all"?<button onClick={()=>setSelectedDate("all")} className="text-indigo-500 hover:underline font-semibold">All dates jouo</button>:"All POs are fully received or completed"}</p></div>
          ):(
            <div className="space-y-4">{filteredPOs.map(po=><POSelectCard key={po.id} po={po} onSelect={handleSelectPO}/>)}</div>
          )}
        </div>
      )}
      ...
      {step===2&&selectedPO&&(
        <div className="space-y-6">
          {selectedPO.hasDamagedPending&&(
            <div className="flex items-start gap-3 p-4 bg-red-50 border border-red-200 rounded-xl">
              <FiAlertTriangle size={15} className="text-red-500 mt-0.5 flex-shrink-0"/>
              <div><p className="text-sm font-black text-red-800">Damage Replacement Invoice</p><p className="text-xs text-red-700 mt-0.5">This PO has <strong>{selectedPO.totalDamagedQty} damaged units</strong> that were not added to stock. Upload the vendor's replacement invoice.</p></div>
            </div>
          )}
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <Card>
                  ...
                  <div className="space-y-1 max-h-48 overflow-y-auto">
                    {selectedPO.items.map((item,idx)=>(
                      <div key={idx} className={`flex items-start text-xs px-3 py-2 rounded-lg gap-3 ${(item.damagedQty||0)>0?"bg-red-50 border border-red-100":"bg-slate-50"}`}>
                        ...
                        <span className="w-20 text-slate-500">{item.orderedQty} {item.unit}</span>
                        {item.totalReceivedQty>0&&<span className="w-20 text-orange-600 font-bold">Recv: {item.totalReceivedQty}</span>}
                        {(item.damagedQty||0)>0&&<span className="flex items-center gap-1 w-16 text-red-600 font-bold"><FiAlertTriangle size={9}/> {item.damagedQty}</span>}
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <Card>
              ...
              <div className="p-6 space-y-4">
                ...
                {duplicateWarning&&<div className="p-3 bg-red-50 border border-red-200 rounded-lg flex items-start gap-2"><FiAlertTriangle size={14} className="text-red-500 mt-0.5 flex-shrink-0"/><p className="text-xs font-bold text-red-700">{duplicateWarning}</p></div>}
                <Input label="Invoice Number" value={invoiceNo} onChange={e=>handleInvoiceNoChange(e.target.value)} placeholder="Auto-filled from Excel or enter manually" required/>
                <Input label="Invoice Date" type="date" value={invoiceDate} onChange={e=>setInvoiceDate(e.target.value)} required/>
                {excelParsed&&<div className="p-3 bg-blue-50 border border-blue-200 rounded-lg"><p className="text-xs font-bold text-blue-700">📦 {receivedItems.filter(i=>i.matchedFromExcel).length} items matched from Invoice Excel</p>{receivedItems.filter(i=>!i.matchedFromExcel).length>0&&<p className="text-xs text-orange-600 mt-1">⚠️ {receivedItems.filter(i=>!i.matchedFromExcel).length} PO items not found in Invoice</p>}</div>}
              </div>
            </Card>
          </div>
          <POHistoryTimeline selectedPO={selectedPO} linkedInvoices={linkedInvoices} loadingHistory={loadingHistory}/>
      ...
      {step===3&&selectedPO&&(
        <Card>
          ...
          <div className="p-6 space-y-4">
            ...
            <div className="p-3 bg-slate-50 rounded-lg"><p className="text-xs font-bold text-slate-600 mb-2">Summary:</p><div className="space-y-1 text-xs"><div className="flex justify-between"><span className="text-slate-500">This Invoice Qty:</span><span className="font-bold text-slate-800">{getTotalNewReceived()} units</span></div><div className="flex justify-between"><span className="text-slate-500">Still Pending:</span><span className={`font-bold ${getTotalShortage()>0?"text-orange-600":"text-emerald-600"}`}>{getTotalShortage()} units</span></div><div className="flex justify-between"><span className="text-slate-500">PO Status After:</span><StatusPill status={livePoStatus}/></div></div></div>
            <div className="flex items-start gap-3 p-3 bg-indigo-50 border border-indigo-200 rounded-lg"><FiAlertCircle size={13} className="text-indigo-500 mt-0.5 flex-shrink-0"/><p className="text-xs text-indigo-700">Clicking <strong>"Submit for Store QC"</strong> will send this invoice to the Store team for quality verification. Stock will update only after their approval.</p></div>
          </div>
        </Card>
      )}
      ...
      {step===4&&selectedPO&&(
        <>
          ...
          {isStoreApproved&&(
            <div className="space-y-5">
              <div className={`flex items-center gap-4 p-4 rounded-xl border ${hasIssues?"border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50":"border-emerald-200 bg-gradient-to-r from-emerald-50 to-teal-50"}`}>
                <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${hasIssues?"bg-amber-500":"bg-emerald-500"}`}>{hasIssues?<FiAlertTriangle size={20} className="text-white"/>:<FiCheck size={20} className="text-white"/>}</div>
                <div className="flex-1 min-w-0"><p className={`text-sm font-black ${hasIssues?"text-amber-800":"text-emerald-800"}`}>{hasIssues?"Quality Check: Approved with Issues":"Quality Check Approved by Store"}</p><p className={`text-xs mt-0.5 ${hasIssues?"text-amber-600":"text-emerald-600"}`}>{storeQcApprovedBy?`Approved by ${storeQcApprovedBy}`:"Items verified by store team."}{storeQcApprovedAt?` · ${formatDateTime(storeQcApprovedAt)}`:""}</p></div>
                <StatusPill status="received"/>
              </div>
              {hasIssues&&(
                <div className="flex items-start gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl">
                  <FiAlertTriangle size={15} className="text-amber-500 mt-0.5 flex-shrink-0"/>
                  <div><p className="text-sm font-black text-amber-800">Damage Noted by Store Team</p><p className="text-xs text-amber-700 mt-1">Damaged units are <strong>not added to stock</strong> and will remain pending from vendor.</p>
                  {damagedItemsList.length>0&&<div className="mt-2 space-y-1">{damagedItemsList.map((item,i)=><div key={i} className="flex items-center gap-2 text-xs text-amber-800"><span className="font-bold font-mono bg-amber-100 px-1.5 py-0.5 rounded">{item.productCode}</span><span>— <strong>{item.damagedQty}</strong> units damaged</span>{item.issueDetail&&<span className="text-amber-600 italic">({item.issueDetail})</span>}</div>)}</div>}</div>
                </div>
              )}
              <Card>
                <div className="flex items-center justify-between px-6 pt-5 pb-3 border-b border-slate-50"><div><h3 className="text-sm font-black text-slate-800">Invoice Summary</h3><p className="text-[11px] text-slate-400 mt-0.5">Review before final submission</p></div><span className="px-3 py-1 text-[10px] font-black rounded-full bg-indigo-50 text-indigo-700 border border-indigo-200 uppercase">Ready to Submit</span></div>
                <div className="p-6 grid grid-cols-2 md:grid-cols-3 gap-x-8 gap-y-4">
              ...
              <div className="flex items-start gap-3 p-4 rounded-xl bg-amber-50 border border-amber-200"><FiAlertCircle size={15} className="text-amber-600 mt-0.5 flex-shrink-0"/><p className="text-xs text-amber-800">Once submitted, this invoice will be <strong>locked for editing</strong>. Stock records are already updated by Store.</p></div>
            </div>
          )}
        </>
      )}
      ...
      {step===4&&<div className="flex justify-end gap-3">{isStoreApproved&&<BtnPrimary onClick={handleFinalSubmit} disabled={uploading}>{uploading?(<span className="flex items-center gap-2"><span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin"/>Processing...</span>):"✓ Confirm Receipt"}</BtnPrimary>}</div>}
          ...
          <Card>
            <div className="p-10 text-center">
              ...
              <p className="text-sm text-slate-500 mb-8 max-w-md mx-auto">
                Store QC approved. Material received and stock updated successfully.
              </p>
              ...
              <div className="flex items-center justify-center gap-3 flex-wrap mb-8">
                <div className="flex items-center gap-2 px-4 py-2 bg-slate-100 rounded-full">
                  ...
                  <span className="text-xs font-black text-slate-800">{selectedPO.poNumber}</span>
                </div>
                <div className="flex items-center gap-2 px-4 py-2 bg-slate-100 rounded-full">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">Invoice</span>
                  <span className="text-xs font-black text-slate-800">{invoiceNo || "—"}</span>
                </div>
                <div className="flex items-center gap-2 px-4 py-2 bg-slate-100 rounded-full">
                  <span className="text-[10px] font-bold text-slate-400 uppercase">Units</span>
                  <span className="text-xs font-black text-slate-800">{getTotalNewReceived()}</span>
                </div>
              </div>
              ...
              {getTotalShortage() > 0 && (
                <div className="max-w-sm mx-auto mb-6 flex items-start gap-3 p-3 bg-orange-50 border border-orange-200 rounded-xl text-left">
                  ...
                  <p className="text-xs text-orange-700">
                    <strong>{getTotalShortage()} units</strong> still pending from vendor.
                    Upload another invoice when received.
                  </p>
                </div>
              )}
              ...
              <div className="flex items-center justify-center gap-3 flex-wrap">
                ...
                <BtnSecondary onClick={() => {
                  ...
                  setInvoiceExcelFile(null); setInvoiceNo(""); setInvoiceHeader(null);
                  setLinkedInvoices([]); setDuplicateWarning(""); setSavedInvoiceId(null);
                }}>Upload Another Invoice</BtnSecondary>
