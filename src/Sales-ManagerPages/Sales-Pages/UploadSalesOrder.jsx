...
export default function UploadSalesOrder() {
      ...
      try {
              ...
              if (cell?.v) {
                ...
                for (const kw of keywords) {
                  if (val.includes(kw.toLowerCase())) {
                    const right =
                      sheet[XLSX.utils.encode_cell({ r, c: c + 1 })];
                    const below =
        ...
        for (let c = 0; c <= range.e.c; c++) {
          ...
          if (cell?.v) {
            ...
            if (val === "unit" || val === "uom" || val.includes("unit"))
              unitCol = c;
          }
        }
        ...
        for (let r = tableStartRow + 2; r <= range.e.r; r++) {
          ...
          parsedItems.push({
            slNo:
              sheet[XLSX.utils.encode_cell({ r, c: 0 })]?.v ||
              parsedItems.length + 1,
            productCode:
              partCol >= 0
                ? sheet[XLSX.utils.encode_cell({ r, c: partCol })]?.v || ""
                : "",
            description: String(descCell.v),
            quantity:
              qtyCol >= 0
                ? parseFloat(
                    sheet[XLSX.utils.encode_cell({ r, c: qtyCol })]?.v || 0,
                  )
                : 0,
           unit: unitCol >= 0
  ? String(sheet[XLSX.utils.encode_cell({ r, c: unitCol })]?.v || "nos").trim().toUpperCase()
  : "nos",
            hsnSac:
              hsnCol >= 0
                ? sheet[XLSX.utils.encode_cell({ r, c: hsnCol })]?.v || ""
                : "",
          });
        }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const customer =
        excelHeader?.consignee || excelHeader?.buyer || "Unknown";
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-0.5">
            Upload Excel → Stock auto-reserved
          </p>
        </div>
                ...
                <tbody className="divide-y divide-slate-100">
                  {items.map((item, idx) => (
                    <tr key={idx} className="hover:bg-slate-50">
                      ...
                      <td className="px-4 py-3 font-bold text-center text-indigo-700">
                        {item.quantity}
                      </td>
              ...
              {stockAlerts.length > 0 && (
                <div className="mt-3 p-3 bg-orange-50 rounded-lg border border-orange-200">
                  <p className="text-orange-700 font-bold text-xs mb-2">
                    ⚠️ {stockAlerts.length} item(s) have stock shortage —
                    backorder created:
                  </p>
                  {stockAlerts.map((a, i) => (
