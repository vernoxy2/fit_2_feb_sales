...
export default function LowStockManagement() {
  ...
  const availableUnits = [
    ...new Set(allProducts.map((p) => p.unit).filter(Boolean)),
  ].sort();
  useEffect(() => {
    const fetchData = async () => {
      ...
      cats.forEach((cat) => {
        (cat.subcategories || []).forEach((sub) => {
          ...
          const liveStock =
            nameKey in stockMap ? stockMap[nameKey] : (sub.currentStock ?? 0);
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            View and manage products below threshold
          </p>
        </div>
        <button
      ...
      {Object.keys(groupedProducts).length === 0 ? (
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
          ...
          <p className="text-sm text-slate-600">
            No products below threshold in selected category
          </p>
        </div>
      ) : (
                    ...
                    return (
                      <tr key={product.id} className="hover:bg-slate-50">
                        ...
                        <td className="px-6 py-4 text-center">
                          <span
                            className={`px-2 py-1 text-xs font-bold rounded-full border ${status.bgColor} ${status.textColor} ${status.borderColor}`}
                          >
                            {status.label}
                          </span>
                        </td>
