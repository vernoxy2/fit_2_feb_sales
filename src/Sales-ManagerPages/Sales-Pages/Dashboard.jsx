...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // 1. stockCategories — priority sort applied here
    const unsubCat = onSnapshot(collection(db, "stockCategories"), (snap) => {
      const data = snap.docs.map((d) => ({ id: d.id, ...d.data() }));

      const sortedCategories = [
        // First: priority categories in exact order
        ...PRIORITY_CATEGORIES
          .map((name) =>
            data.find(
              (c) => c.name?.toUpperCase().trim() === name
            )
          )
          .filter(Boolean),
        // Then: all remaining categories not in priority list
        ...data.filter(
          (c) =>
            !PRIORITY_CATEGORIES.includes(c.name?.toUpperCase().trim())
        ),
      ];
