...
import {
  FiHome, FiFileText, FiBell, FiTruck, FiDollarSign,
  FiClock, FiArchive, FiShoppingCart, FiUpload, FiFile,
  FiBarChart2, FiPackage, FiAlertTriangle, FiTrendingUp,
  FiLayers, FiSettings, FiClipboard, FiInbox, FiRefreshCw,
  FiMenu, FiX,
} from "react-icons/fi";
import logo from "../../../assets/logo.svg";
...
export default function Sidebar({ collapsed, setCollapsed }) {
  ...
  const navSections = [
    ...
    },
    {
      ...
      items: [
        ...
        { to: "/sales/recieved-on-challan",     icon: FiInbox,        label: "Recieved on Challan",   badge: 2,    badgeColor: "amber" },
  ...
  return (
    ...
    >
      {/* ── Logo + Hamburger ── */}
      <div className="h-16 border-b border-slate-200 flex items-center flex-shrink-0 px-3 gap-2 justify-between">
        {/* Logo area */}
        ...
        {/* Collapsed: only logo icon */}
        {collapsed && (
          <img src={logo} alt="Fib2Fab" className="h-9 mx-auto flex-shrink-0" />
        )}
        ...
        {navSections.map((section, idx) => (
          <div key={idx} className={idx > 0 ? "mt-5" : ""}>
            ...
            {section.title && collapsed && idx > 0 && (
              <div className="border-t border-slate-100 mb-3 mx-2" />
            )}
                ...
                >
                  {({ isActive }) => (
                    <>
                      ...
                      {collapsed && item.badge && (
                        <span className={`absolute top-1.5 right-1.5 w-2 h-2 rounded-full flex-shrink-0
