...
async function updateStockForReturn(
  item,
  noteRef,
  noteNumber,
  qty,
  ...
  const stockSnap = await getDocs(
    query(collection(db, DB_STOCK), where("productCode", "==", key)),
  );
...
function NoteDetail({
  ...
  // );
  const [verifyRows, setVerifyRows] = useState(() => {
    // ✅ Use verifiedItems when already processed — it has issue/issueQty data
    const source =
      note.verifiedItems && note.verifiedItems.length > 0
        ? note.verifiedItems
        : note.items || [];
    ...
    return source.map((item) => ({
      ...
      invoiceQty: parseFloat(item.invoiceQty) || 0,
      returnQty: parseFloat(item.returnQty) || 0,
      verifyQty: parseFloat(item.verifyQty ?? item.returnQty ?? 0),
      issue: item.issue || "",
  ...
  const totalDispatched = verifyRows.reduce(
    (s, r) => s + (r.invoiceQty || 0),
    0,
  );
  const totalReturn = verifyRows.reduce((s, r) => s + (r.returnQty || 0), 0);
  const totalVerified = verifyRows.reduce(
    (s, r) => s + (parseFloat(r.verifyQty) || 0),
    0,
  );
  const totalIssue = verifyRows.reduce(
    ...
    <div className="space-y-5">
        ...
        <div className="flex items-start justify-between gap-4 flex-wrap">
          <div className="flex-1 min-w-0">
            ...
            <p className="text-sm text-indigo-200">
              Note:{" "}
              <strong className="text-white">{note.noteNumber || "—"}</strong>
            </p>
      ...
      <div className="grid grid-cols-5 gap-3">
        {[
          ...
          <div
            key={label}
            className={`border rounded-xl p-3 text-center ${bg}`}
          >
      ...
      {hasIssues && !isProcessed && (
        <div className="flex items-start gap-3 p-3 bg-orange-50 border border-orange-300 rounded-xl">
          ...
          <div className="text-xs text-orange-700 space-y-0.5">
            ...
            <p>
              <strong>Approve</strong> → Clean qty ({totalClean}) will be added
              to stock, issue badge set. &nbsp;
              <strong>Passed with Issues</strong> → Only clean qty added to
              stock, issue qty ({totalIssue}) will remain pending on both
              panels.
            </p>
          </div>
        </div>
          ...
          <div>
            ...
            {note.storeRemarks && (
              <p className="text-xs text-slate-500 mt-0.5">
                Store remarks: {note.storeRemarks}
              </p>
            )}
          </div>
        </div>
          ...
          <div className="px-3 py-2.5 bg-slate-50 border-b border-slate-200 flex items-center gap-2">
            ...
            <div className="min-w-0">
              <p className="text-xs font-bold text-slate-700">
                Sales Dispatched Items
              </p>
              <p className="text-[10px] text-slate-400">
                Fixed — approved invoice
              </p>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-[11px]">
              <thead className="bg-slate-50 border-b border-slate-100">
                <tr>
                  {["No", "Description", "Part No", "Disp Qty", "Unit"].map(
                    (h) => (
                      <th
                        key={h}
              ...
              <tbody className="divide-y divide-slate-50">
                {(note.items || []).map((item, i) => (
                  <tr key={i} className="hover:bg-slate-50/60">
                    ...
                    <td className="px-2 py-2 font-black text-slate-800 text-center">
                      {item.invoiceQty || 0}
                    </td>
                    <td className="px-2 py-2 text-slate-400 uppercase text-[10px]">
                      {item.unit || "pcs"}
                    </td>
                  </tr>
                ))}
            ...
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-xs font-bold text-red-700">
                  {isProcessed
                    ? "Verified Return Items"
                    : "Verify Return Items — Store QC"}
                </p>
                <p className="text-[10px] text-red-400">
                  {isProcessed
                    ? "Read only — processed"
                    : "Adjust qty · set issue if any damage/shortage found"}
                </p>
              </div>
            </div>
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-xs">
              <thead className="bg-red-50/50 border-b border-red-100">
                <tr>
                  <th className="px-2 py-2.5 text-left font-bold text-slate-400 w-7 whitespace-nowrap">
                    No
                  </th>
                  <th className="px-2 py-2.5 text-left font-bold text-slate-400 whitespace-nowrap">
                    Description
                  </th>
                  <th className="px-2 py-2.5 text-left font-bold text-slate-400 w-24 whitespace-nowrap">
                    Part No
                  </th>
                  <th className="px-2 py-2.5 text-center font-bold text-slate-500 w-12 whitespace-nowrap">
                    Inv
                    <br />
                    Qty
                  </th>
                  <th className="px-2 py-2.5 text-center font-bold text-blue-500 w-12 whitespace-nowrap">
                    Return
                    <br />
                    Qty
                  </th>
                  <th className="px-8 py-2.5 text-center font-bold text-red-500 w-14 whitespace-nowrap">
                    Verify
                    <br />
                    Qty
                  </th>
                  ...
                  <th className="px-2 py-2.5 text-left font-bold text-orange-500 w-24 whitespace-nowrap">
                    Issue
                  </th>
                  <th className="px-8 py-2.5 text-center font-bold text-amber-600 w-12 whitespace-nowrap">
                    Issue
                    <br />
                    Qty
                  </th>
                  <th className="px-2 py-2.5 text-center font-bold text-emerald-600 w-14 whitespace-nowrap">
                    Physical
                    <br />
                    Qty
                  </th>
              ...
              <tbody className="divide-y divide-red-50">
                {verifyRows.map((row, i) => {
                  ...
                  const isMismatch =
                    !isProcessed && hasVerify && verifyQty !== returnQty;
                  ...
                  return (
                    ...
                    >
                      ...
                      <td className="px-2 py-2 font-bold text-slate-600 text-center">
                        {invoiceQty}
                      </td>

                      <td className="px-2 py-2 text-center">
                        <span
                          className={`text-sm font-black ${returnQty > 0 ? "text-blue-600" : "text-slate-300"}`}
                        >
                          {returnQty || "—"}
                        </span>
                      </td>
                      ...
                      <td className="px-3 py-2">
                        {isProcessed ? (
                          <span
                            className={`text-sm font-black block text-center ${hasVerify ? "text-red-600" : "text-slate-300"}`}
                          >
                            {verifyQty || "—"}
                          </span>
                        ) : (
                          <div className="relative">
                            <input
                              ...
                              placeholder="0"
                              onChange={(e) =>
                                updateRow(i, "verifyQty", e.target.value)
                              }
                      ...
                      <td className="px-2 py-2">
                        {isProcessed ? (
                          ...
                          <select
                            ...
                            disabled={!hasVerify}
                            onChange={(e) =>
                              updateRow(i, "issue", e.target.value)
                            }
                      ...
                      {/* Issue Qty — ADD THIS CELL */}
                      <td className="px-1 py-2">
                        {isProcessed ? (
                          <span
                            className={`text-sm font-black block text-center ${issueQty > 0 ? "text-amber-600" : "text-slate-200"}`}
                          >
                            {issueQty > 0 ? issueQty : "—"}
                          </span>
                        ) : hasIssue && hasVerify ? (
                          <input
                            ...
                            placeholder="0"
                            onChange={(e) =>
                              updateRow(i, "issueQty", e.target.value)
                            }
                          ...
                          <span className="text-slate-200 text-[10px] block text-center">
                            —
                          </span>
                        )}
                      </td>
              ...
              {!isProcessed && (
                <tfoot className="bg-red-50/50 border-t border-red-200">
                  <tr>
                    ...
                    <td className="px-2 py-2 text-center font-black text-slate-600">
                      {totalDispatched}
                    </td>
        ...
        <div className="bg-white border border-slate-200 rounded-xl p-5 space-y-4">
          ...
          <div className="flex items-start gap-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
            ...
            <div className="text-xs text-blue-700 space-y-0.5">
              <p>
                <strong>Approve</strong> → All verified qty added to stock
                (clean + issue both), issue badge set
              </p>
              <p>
                <strong>Passed with Issues</strong> → Only clean qty (
                {totalClean}) added to stock, issue qty ({totalIssue}) remains
                pending — status stays pending on Sales panel
              </p>
              <p>
                <strong>Reject</strong> → No stock update
              </p>
            </div>
          </div>
          <div>
            <label className="text-xs font-bold text-slate-500 uppercase tracking-wider block mb-1.5">
              Store Remarks (optional)
            </label>
            <textarea
              rows={2}
              value={storeRemarks}
          ...
          <div className="flex items-center justify-end gap-3 pt-2 border-t border-slate-100">
            <button
              onClick={() => onReject(note.id, storeRemarks)}
              disabled={saving}
            ...
            <button
              onClick={() =>
                onPassWithIssues(note.id, storeRemarks, note, verifyRows)
              }
              disabled={saving}
            ...
            <button
              onClick={() => onApprove(note.id, storeRemarks, note, verifyRows)}
              disabled={saving}
  ...
  const handleApprove = async (noteId, remarks, note, verifyRows) => {
    ...
    try {
      for (const row of verifyRows) {
        ...
        if (cleanQty > 0)
          await updateStockForReturn(
            row,
            note.linkedInvoiceNo || noteId,
            note.noteNumber || noteId,
            cleanQty,
            false,
          );
    ...
    try {
      for (const row of verifyRows) {
        ...
        if (issueQty > 0 && row.issue && row.partNo) {
          const stockSnap = await getDocs(
            query(
              collection(db, DB_STOCK),
              where("productCode", "==", row.partNo.trim()),
            ),
          );
          if (!stockSnap.empty) {
      ...
      <div className="space-y-6">
        <div>
          <h2 className="text-xl font-black text-slate-800">
            Debit Notes — Store QC
          </h2>
          <p className="text-xs text-slate-400 mt-0.5">
            Review return requests from Sales team
          </p>
        </div>
        <NoteDetail
          note={selectedNote}
      ...
      {loading ? (
        ...
        <div className="bg-white rounded-xl border border-slate-200 p-12 text-center">
          ...
          <p className="text-sm font-bold text-slate-500">
            {tab === "pending" ? "No Pending QC Requests" : "No Notes Found"}
          </p>
          <p className="text-xs text-slate-400 mt-1">
            {tab === "pending" ? "All requests processed" : "Check other tabs"}
          </p>
        </div>
      ) : (
          ...
          {tab === "pending" && pendingNotes.length > 0 && (
            <div className="flex items-center gap-3 p-3 bg-amber-50 border border-amber-200 rounded-xl">
              ...
              <p className="text-xs text-amber-700">
                <strong>{pendingNotes.length} return request(s)</strong> waiting
                for physical inspection and QC approval.
              </p>
            </div>
          )}
          {displayNotes.map((note) => {
            const totalReturn = (note.items || []).reduce(
              (s, i) => s + (parseFloat(i.returnQty) || 0),
              0,
            );
            ...
            return (
              <div
                ...
                className={`bg-white border rounded-xl p-4 cursor-pointer transition-all hover:shadow-md ${
                  isPending
                    ? "border-l-4 border-l-amber-400 border-amber-200 bg-amber-50/10"
                    : note.status === "approved"
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 flex-wrap mb-1">
                      ...
                      <p className="text-sm font-black text-slate-800">
                        {note.customer || note.vendorName || "—"}
                      </p>
                      <StatusPill status={note.status} />
                      {isPending && (
                        <span className="px-2 py-0.5 text-[10px] font-bold bg-amber-100 text-amber-700 border border-amber-200 rounded-full animate-pulse">
                          Awaiting Inspection
                        </span>
                      )}
                      {isWithIssues && (
                    ...
                    <div className="flex flex-wrap gap-1.5 mt-2">
                      {(note.items || []).slice(0, 3).map((item, i) => (
                        <span
                          key={i}
                          className="text-[10px] bg-red-50 border border-red-200 text-red-700 px-2 py-0.5 rounded-md font-bold"
                        >
