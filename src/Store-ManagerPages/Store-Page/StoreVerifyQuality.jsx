  ...
  for (const item of items) {
    ...
    if (snap.empty) {
      ...
      await updateDoc(doc(db, "stock", sd.id), {
        ...
        excess: excessQty,
        damagedQty: (parseFloat(sdata.damagedQty || 0) + damagedQty),
        hasIssue:   damagedQty > 0 || (sdata.damagedQty || 0) > 0, 
        lastUpdated: now,
        ledger: [
          ...(sdata.ledger || []),
          {
    ...
    const verifyAndRestore = async () => {
      try {
        ...
        const invSnap = await getDoc(
          doc(db, "excelupload", d.selectedInvoice.id),
        );
        if (!invSnap.exists()) {
          localStorage.removeItem("storeQcDraft");
          return;
              ...
              {pagedItems.map((item) => {
                ...
                return (
                  <div
                    ...
                    className={`p-4 border rounded-lg ${itemStatus === "complete" ? "border-emerald-200 bg-emerald-50/30" : itemStatus === "excess" ? "border-purple-200 bg-purple-50/30" : itemStatus === "partial" ? "border-orange-200 bg-orange-50/30" : "border-slate-200"}`}
                  >
                          ...
                          <div className="flex-1">
                            <p className="text-[10px] text-red-600 font-bold mb-1 uppercase tracking-wide">
                              ...
                              <span className="normal-case font-normal text-red-400">
                                (tracking only — not deducted from stock)
                              </span>
                            </p>
                            <input
                              type="number"
              ...
              {qualityCheck === "failed" ? (
                ...
                <div className="flex items-start gap-3 p-3 bg-emerald-50 border border-emerald-200 rounded-xl">
                  ...
                  <p className="text-xs text-emerald-800">
                    ...
                    <strong>Sales team will be notified</strong>.
                  </p>
                </div>
              )}
            </div>
