...
// // ✅ Fully uncontrolled input — no React re-render issues
// function QtyInput({ initialValue, max, isShort, onChange }) {
...
//       className={`w-16 h-7 border rounded-lg px-2 text-xs font-bold text-center focus:outline-none focus:ring-2 ${
//         isShort
//           ? "border-orange-300 bg-orange-50 text-orange-700 focus:ring-orange-300"
//           : "border-emerald-200 bg-emerald-50/40 text-emerald-700 focus:ring-emerald-300"
//       }`}
//     />
//   );
// }
...
//       soItemsRef.current = mappedItems;
...
//         item.productCode === productCode ? { ...item, ...changes } : item,
//       );
//       soItemsRef.current = updated; // ✅ always in sync
//       return updated;
//     });
//   };
...
//                               className="ml-4 flex items-center gap-1.5 px-4 py-2 bg-emerald-100 text-emerald-700 border border-emerald-200 text-xs font-bold rounded-lg hover:bg-emerald-200 transition-colors whitespace-nowrap"
//                             >
...
//                               onClick={() => handleSelectSO(so)}
//                               disabled={loadingSO2}
//                               className={`ml-4 px-4 py-2 text-white text-xs font-bold rounded-lg transition-colors whitespace-nowrap flex items-center gap-1.5 disabled:opacity-60 ${hasQcIssues ? "bg-orange-500 hover:bg-orange-600" : "bg-violet-600 hover:bg-violet-700"}`}
//                             >
//                               <FiShield size={12} />
//                               {loadingSO2
//                                 ? "Loading..."
...
//                             }}
//                             className={`ml-4 px-4 py-2 text-white text-xs font-bold rounded-lg transition-colors whitespace-nowrap flex items-center gap-1.5 ${isApproved ? "bg-emerald-600 opacity-60 cursor-not-allowed" : hasIssues ? "bg-red-600 hover:bg-red-700" : "bg-emerald-600 hover:bg-emerald-700"}`}
//                           >
//                             <FiShield size={12} />
//                             {isApproved
//                               ? "✅ Approved"
...
//             </Card>
//             {/* <Card>
//             <CardHeader title="Verify Stock Availability11" subtitle={`${soTotalReady} units ready · ${soItems.length} items`} />
...
//                       <th className="text-left px-3 py-2.5 font-bold text-slate-500 uppercase tracking-wide w-10">
//                         #
//                       </th>
...
//                         Description
//                       </th>
//                       <th className="text-center px-3 py-2.5 font-bold text-slate-500 uppercase tracking-wide w-16">
//                         Unit
//                       </th>
...
//           const isShort = ready < ordered;
//           return (
//             <tr key={item.productCode} className={`hover:bg-slate-50 transition-colors ${isShort ? "bg-orange-50/40" : ""}`}>
//               <td className="px-3 py-2.5 text-slate-400 font-bold">{idx + 1}</td>
//               <td className="px-3 py-2.5 font-mono font-bold text-slate-800">{item.productCode}</td>
//               <td className="px-3 py-2.5 text-slate-600 max-w-[200px] truncate">{item.description}</td>
//               <td className="px-3 py-2.5 text-center text-slate-500">{item.unit || "pcs"}</td>
...
//                     if (item.issue === "damage") {
//                       const autoDmg = Math.max(0, ordered - newReady);
//                       updateSoItem(item.productCode, { readyQty: newReady, damagedQty: autoDmg, issueDetail: `Damaged: ${autoDmg} units` });
//                     } else {
//                       const autoIssue = newReady < ordered ? "shortage" : "";
//                       const autoDetail = newReady < ordered ? `Short: ${ordered - newReady} units` : "";
//                       updateSoItem(item.productCode, { readyQty: newReady, issue: autoIssue, issueDetail: autoDetail });
//                     }
//                   }}
//                   className={`w-16 h-7 border rounded-lg px-2 text-xs font-bold text-center focus:outline-none focus:ring-2 ${
...
//                           className={`hover:bg-slate-50 transition-colors ${isShort ? "bg-orange-50/40" : ""}`}
//                         >
...
//                               max={avail}
//                               defaultValue={ready}
//                               key={`${item.productCode}-${item.readyQty ?? "init"}`}
//                               onBlur={(e) => {
//                                 const newReady = Math.min(
//                                   Math.max(0, parseFloat(e.target.value) || 0),
//                                   avail,
//                                 );
//                                 e.target.value = newReady;
//                                 if (item.issue === "damage") {
//                                   const autoDmg = Math.max(
//                                     0,
//                                     ordered - newReady,
//                                   );
...
//                                     readyQty: newReady,
//                                     damagedQty: autoDmg,
//                                     issueDetail: `Damaged: ${autoDmg} units`,
//                                   });
//                                 } else {
//                                   const autoIssue =
...
// function QtyInput({ initialValue, max, onChange }) {
//   const inputRef = useRef(null);
...
//   useEffect(() => {
//     if (inputRef.current) {
//       inputRef.current.value = String(initialValue ?? 0);
//     }
...
//         e.target.value = raw;
//         const parsed = raw === "" ? 0 : parseInt(raw, 10) || 0;
//         const clamped = Math.min(parsed, maxRef.current ?? 99999);
//         // update input style directly — no re-render
//         if (clamped < (initialValue ?? 0)) {
//           e.target.className =
//             e.target.className
  ...
  return (
    <input
      ...
      onInput={(e) => {
        ...
        e.target.value = raw;
        const parsed = raw === "" ? 0 : parseInt(raw, 10) || 0;
        onChange(parsed); // ✅ no clamping — pass exact value
      }}
      onFocus={(e) => e.target.select()}
      className="w-16 h-7 border border-emerald-200 rounded-lg px-2 text-xs font-bold text-center bg-emerald-50/40 text-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-300"
    />
...
export default function StoreVerifyQuality() {
  ...
  const [selectedSO, setSelectedSO] = useState(null);
  const [soItems, setSoItems] = useState([]);
  const soItemsRef = useRef([]); // ✅ always in sync with soItems
  const [loadingSO2, setLoadingSO2] = useState(false);
  const [loadingPO, setLoadingPO] = useState(false);
  const [selectedInvoice, setSelectedInvoice] = useState(null);
  const [selectedPO, setSelectedPO] = useState(null);
  ...
  const updateSoItem = (productCode, changes) => {
    setSoItems((prev) => {
      ...
      soItemsRef.current = updated; // ✅ ref always updated
      return updated;
    });
  };
  ...
  const handleSelectSO = async (so) => {
    ...
    try {
      ...
      setSoItemsSync(mappedItems); // ✅ use sync setter
      setQualityCheck("passed");
      setRemarks("");
      setQcMode("so");
      setCurrentPage(1);
  ...
  const handleSOSubmit = async () => {
    ...
    try {
      const now = new Date().toISOString();
      const currentItems = soItemsRef.current; // ✅ use ref for latest values
      ...
      {step === 2 &&
        ...
        selectedSO.soStatus !== "complete" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            {/* ✅ RIGHT: Simple table with QtyInput */}
            <Card>
              <CardHeader
                title="Items for Dispatch"
                subtitle={`${soItems.length} items · ${soTotalReady} units ready`}
                        ...
                        >
                          ...
                          <QtyInput
                            ...
                            onChange={(newReady) => {
                              ...
                              updateSoItem(item.productCode, {
                                readyQty: newReady,
                                issue: item.issue === "damage" ? item.issue : autoIssue,
                                issueDetail: item.issue === "damage" ? item.issueDetail : autoDetail,
                                ...(item.issue === "damage" && { damagedQty: Math.max(0, ordered - newReady) }),
                              });
                            }}
                          />
                        </td> */}
                          <td className="px-3 py-2.5 text-center">
                            <QtyInput
                              ...
                              max={avail}
                              // onChange={(newReady) => {
                              //    console.log("QtyInput onChange called:", item.productCode, newReady); 
                              //   const autoIssue =
                              //     newReady < ordered ? "shortage" : "";
                              //   const autoDetail =
                              //     newReady < ordered
    ...
    ...(item.issue === "damage" && { damagedQty: Math.max(0, ordered - newReady) }),
      ...
      {step === 3 && qcMode === "so" && selectedSO && (
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          {/* SO Step 3 Right — Confirm Summary Table */}
          <Card>
            <CardHeader
              title="Confirm Summary"
              subtitle="Review before final approval"
            ...
            <div className="p-6 space-y-4">
              ...
              <div className="overflow-auto max-h-64">
                <table className="w-full text-xs border-collapse">
                  ...
                  <tbody className="divide-y divide-slate-100">
                    {soItems.map((item, idx) => {
                      console.log("Step3 item:", item.productCode, item.readyQty);
                      const ready = item.readyQty ?? 0;
                      const st = getSoItemStatus(item);
